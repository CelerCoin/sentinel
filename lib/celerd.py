...
class CelerDaemon():
    def __init__(self, **kwargs):
        host = config.sentinel_cfg.get('celer_host', None)
        if host is None:
        ...
        # memoize calls to some celerd methods
    ...
    @classmethod
    def from_celer_conf(self, celer_dot_conf):
        from celer_config import CelerConfig
        config_text = CelerConfig.slurp_config_file(celer_dot_conf)
        creds = CelerConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from celerlib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = celerlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = celerlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = celerlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_celerd_proto_version_with_sentinel_ping)
