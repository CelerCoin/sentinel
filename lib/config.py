...
sentinel_cfg = CelerConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_celerd_proto_version_with_sentinel_ping = 70207

def get_celer_conf():
    home = os.environ.get('HOME')

    if sys.platform == 'darwin':
        celer_conf = os.path.join(home, "Library/Application Support/CelerCore/celer.conf")
    elif sys.platform == 'win32':
        celer_conf = os.path.join(os.environ['APPDATA'], "CelerCore/Celer.conf")
    else:
        celer_conf = os.path.join(home, ".celercore/celer.conf")
        celer_conf = sentinel_cfg.get('celer_conf', celer_conf)
