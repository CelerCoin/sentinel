...
def is_valid_celer_address(address, network='mainnet'):
    ...
    celer_version = 140 if network == 'testnet' else 70
    ...
    try:
        ...
        # rescue from exception, not a valid Celer address
        return False

    if (address_version != celer_version):
        return False
...
# shims 'til we can fix the celerd side
def SHIM_serialise_for_celerd(sentinel_hex):
    from models import CELERD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = CELERD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in celerd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # celerd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    celerd_hex = serialise(obj)
    return celerd_hex


# shims 'til we can fix the celerd side
def SHIM_deserialise_from_celerd(celerd_hex):
    from models import CELERD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(celerd_hex)
...
def did_we_vote(output):
    ...
    _, config_filename = os.path.split(config.celer_conf)
    try:
