...
class CelerConfig():
    ...
    def slurp_config_file(self, filename):
        # read celer.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is celer.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from celer.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Celer defaults...
        default_port = 7777 if (network == 'mainnet') else 17777

        # use default port for network if not specified in celer.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from celer.conf
