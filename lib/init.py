...
def has_celer_conf():
    ...
    valid_celer_conf = False

    try:
        f = io.open(config.celer_conf)
        valid_celer_conf = True
    except IOError as e:
        print(e)

    return valid_celer_conf
...
def main():
    ...
    if not has_celer_conf():
        print("CelerCore must be installed and configured, including JSONRPC access in celer.conf")
        sys.exit(1)
