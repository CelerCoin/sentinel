...
class GovernanceObject(BaseModel):
    ...
    # sync celerd gobject list with our local relational DB backend
    @classmethod
    def sync(self, celerd):
        golist = celerd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_celerd(celerd, item)
    ...
    @classmethod
    def import_gobject_from_celerd(self, celerd, rec):
        import decimal
        import celerlib
        import inflection
        ...
        # shim/celerd conversion
        object_hex = celerlib.SHIM_deserialise_from_celerd(object_hex)
        objects = celerlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from celerd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from celerd, with every run
        ...
        # get/create, then sync payment amounts, etc. from celerd - Celerd is the master
        try:
            ...
            if sub.is_valid(*params) is False:
                govobj.vote_delete(celerd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from celerd! %s" % e)
            govobj.vote_delete(celerd)
            return (govobj, None)
    ...
    def vote_delete(self, celerd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(celerd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, celerd, signal, outcome):
        import celerlib
        ...
        output = celerd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = celerlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(celerd, signal)

    def sync_network_vote(self, celerd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = celerd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CELERD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 celer addr, non-multisig
            if not celerlib.is_valid_celer_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Celer address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (celerlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CelerDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import celerlib
        obj_data = celerlib.SHIM_serialise_for_celerd(self.serialise())
    ...
    def prepare(self, celerd):
        try:
            object_hash = celerd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CELERD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not celerlib.is_valid_celer_address(addr, config.network):
    ...
    def hash(self):
        import celerlib
        return celerlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = CELERD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, celerd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - celerd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, celerd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - celerd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, celerd):
        now = int(time.time())
        return (self.created_at < (now - celerd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, celerd):
        if self.is_expired(celerd):
            return False

        return True

    def is_deletable(self, celerd):
        if self.is_expired(celerd):
            return True
